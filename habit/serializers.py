...
class HabitSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Habit
        fields = '__all__'  # Берём все поля модели Habit
    ...
    def validate(self, data):
        ...
        if reward and related_habit:
            raise serializers.ValidationError("Нельзя указывать и награду, и связанную привычку одновременно.")
...
class HabitPublicSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Habit
        fields = ('id', 'place', 'time', 'action', 'is_pleasant', 'periodicity', 'execution_time')
        read_only_fields = fields  # Запретить изменения всех полей